...
| 12 ⬜ | HADES-fpd | R-27 per-rerun Turso fan-out | `@st.cache_data(ttl=60)` wrappers for operators list, templates, title prefs, weekly usage reads on hot pages; session-memo the page-4 dedup lookup keyed on lead-set hash + window. Invalidate operators cache on Operators-page mutations. |
